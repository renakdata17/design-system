...
export const WithCategoryFilter: Story = {
  render: () => {
    ...
    const filtered = selected ? samplePosts.filter((p) => p.category === selected) : samplePosts;
    return (
      <PostList
        title="Blog"
        posts={filtered}
...
export const EmptyState: Story = {
  render: () => (
    <PostList title="Blog" posts={[]} categories={categories} selectedCategory="Product" />
  ),
};
...
export const Mobile: Story = {
  ...
  render: () => <PostList title="Blog" posts={samplePosts.slice(0, 3)} categories={categories} />,
};
