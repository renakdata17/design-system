...
const CopilotPanel = React.forwardRef<React.ComponentRef<typeof SheetContent>, CopilotPanelProps>(
...
const CopilotPanelHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
...
>(({ className, ...props }, ref) => (
  <SheetTitle
    ref={ref}
    className={cn("flex items-center gap-2 text-lg font-semibold text-foreground", className)}
    {...props}
  />
));
...
const CopilotPanelContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, children, ...props }, ref) => (
    <ScrollArea className="flex-1">
      <div ref={ref} className={cn("flex flex-col gap-4 p-6", className)} {...props}>
        {children}
      </div>
    </ScrollArea>
  ),
...
const CopilotPanelFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn("flex flex-col gap-3 border-t px-6 py-4", className)} {...props} />
  ),
);
CopilotPanelFooter.displayName = "CopilotPanelFooter";

export interface CopilotPanelContextProps extends React.HTMLAttributes<HTMLDivElement> {
  icon?: React.ReactNode;
  label: string;
  value?: string;
}

const CopilotPanelContext = React.forwardRef<HTMLDivElement, CopilotPanelContextProps>(
  ({ className, icon, label, value, children, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("flex items-start gap-3 rounded-lg border bg-muted/30 p-3", className)}
      {...props}
    >
      {icon && (
...
export interface CopilotPanelContextListProps extends React.HTMLAttributes<HTMLDivElement> {}

const CopilotPanelContextList = React.forwardRef<HTMLDivElement, CopilotPanelContextListProps>(
  ({ className, children, ...props }, ref) => (
    <div
      ref={ref}
...
const CopilotPanelSuggestion = React.forwardRef<HTMLButtonElement, CopilotSuggestionProps>(
...
export interface CopilotPanelSuggestionsProps extends React.HTMLAttributes<HTMLDivElement> {
  title?: string;
}

const CopilotPanelSuggestions = React.forwardRef<HTMLDivElement, CopilotPanelSuggestionsProps>(
  ({ className, title = "Suggestions", children, ...props }, ref) => (
    <div ref={ref} className={cn("flex flex-col gap-2", className)} {...props}>
      <span className="text-xs font-medium text-muted-foreground">{title}</span>
      <div className="flex flex-wrap gap-2" role="group" aria-label="Action suggestions">
        {children}
      </div>
    </div>
  ),
...
const CopilotPanelDivider = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} role="separator" className={cn("h-px bg-border", className)} {...props} />
  ),
);
CopilotPanelDivider.displayName = "CopilotPanelDivider";

export interface CopilotPanelInputProps extends React.ComponentPropsWithoutRef<typeof ChatInput> {
  onSend?: (value: string) => void;
}

const CopilotPanelInput = React.forwardRef<HTMLTextAreaElement, CopilotPanelInputProps>(
  ({ onSend, ...props }, ref) => (
    <ChatInput ref={ref} onSend={onSend} size="md" placeholder="Ask me anything..." {...props} />
  ),
);
CopilotPanelInput.displayName = "CopilotPanelInput";

export interface CopilotPanelChatHistoryProps extends React.HTMLAttributes<HTMLDivElement> {
  children?: React.ReactNode;
}

const CopilotPanelChatHistory = React.forwardRef<HTMLDivElement, CopilotPanelChatHistoryProps>(
  ({ className, children, ...props }, ref) => (
    <div
      ref={ref}
