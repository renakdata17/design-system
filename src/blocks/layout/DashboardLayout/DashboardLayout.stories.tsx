...
const SampleHeader = () => (
  <div className="flex h-14 items-center gap-4 px-4">
    ...
    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[hsl(var(--la-muted))] text-xs font-medium">JS</div>
  </div>
);

const SampleLeftPanel = ({ title = "Sidebar" }: { title?: string }) => (
  <div className="p-4">
    <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-[hsl(var(--la-muted-foreground))]">{title}</p>
    {["Overview", "Analytics", "Reports", "Customers", "Products", "Settings"].map((item, i) => (
      <button
        key={item}
        type="button"
        className={`mb-1 flex w-full items-center rounded-[--la-radius] px-3 py-2 text-sm transition-colors hover:bg-[hsl(var(--la-accent))] ${i === 0 ? "bg-[hsl(var(--la-accent))] font-medium" : "text-[hsl(var(--la-foreground))]"}`}
      >
        {item}
      </button>
    ))}
...
const SampleRightPanel = () => (
  <div className="p-4">
    <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-[hsl(var(--la-muted-foreground))]">Details</p>
    <div className="space-y-3">
      {["Revenue", "Users", "Orders", "Conversion"].map((label) => (
        <div key={label} className="flex items-center justify-between rounded-[--la-radius] border border-[hsl(var(--la-border))] p-3">
          <span className="text-sm text-[hsl(var(--la-muted-foreground))]">{label}</span>
  ...
  <div className="h-full min-h-64 p-6">
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="rounded-lg border border-[hsl(var(--la-border))] bg-[hsl(var(--la-card))] p-4">
          <p className="text-sm font-medium">Card {i + 1}</p>
          <p className="mt-1 text-2xl font-bold">—</p>
          <p className="mt-1 text-xs text-[hsl(var(--la-muted-foreground))]">{label}</p>
        </div>
      ))}
    </div>
  </div>
